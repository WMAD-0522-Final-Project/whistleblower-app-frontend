  ...
  const handleFileSelect = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
        ...
        axios({
          ...
          headers: { Authorization: getAuthorizationValue(), contentType: 'multipart/form-data'},
          data: formdata,
        });  
    }
    ...
    <div className='gv_cont'>
      ...
      <div className="gv_file-upload">
        <input type="file"  accept="image/png, image/jpeg"  ref={fileInputRef} onChange={handleFileSelect} style={{ display: 'none' }} />
        <button type="button" onClick={handleButtonClick}>
        <div className='gv_img_cont'>
          <div className='img_hover_edit'>
            <p>Edit</p>
          </div>
          <img src={(companyImgChanging.length > 0 )? companyImgChanging : companyData.logoImg } alt="" />
        </div>
        </button>
        </div>
      ...
      <div className='gv_sub_cont gv_themeEditor'>
        <div className='gv_inputs'>
          <div className='th_inputSuperCont'>
            <div className='th_inputSub'>
              <input type="color" id="color" name="color" value={primaryColor}
                onChange={(e)=>{setPrimaryColor(e.target.value)}}/>
              <p className='th_primaryColor_title'>Primary</p>
            </div>
            <input className='th_color_value' value={primaryColor.toUpperCase()}
              onChange={(e)=>{setPrimaryColor(e.target.value)}}></input>
          </div>
          <div className='th_inputSuperCont'>
            <div className='th_inputSub'>
              <input type="color" id="color" name="color" value={secondaryColor}
                onChange={(e)=>{setSecondaryColor(e.target.value)}}/>
              <p className='th_primaryColor_title'>Secondary</p>
            </div>
            <input className='th_color_value' value={secondaryColor.toUpperCase()}
              onChange={(e)=>{setSecondaryColor(e.target.value)}}></input>
          </div>
        </div>
        <div className='gv_side_cont gv_pallete_super'>
          <div className='gv_pallete'>
            ...
            return (
            <ListCustom key={index} bgColor={`${true ? '#FFFFFF' : '#000000' }`} height={40}
              iconColorLeft={item.mainColor} iconColorCenter={item.secondaryColor} onClick={()=> {
      ...
      <div className='gv_submit_cont'>
        <button className='gv_button' onClick={()=>{
